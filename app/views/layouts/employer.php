...
<aside class="sidebar d-none d-lg-flex flex-column">
    ...
    function el(string $href, string $icon, string $label, string $uri): string {
    ...
    <nav class="sidebar-nav flex-grow-1">
        ...
        <ul class="list-unstyled mb-0">
            <?= el(url('/employer/dashboard'),   'speedometer2',   'Dashboard',       $uri) ?>
            <?= el(url('/employer/jobs'),        'briefcase',      'My Jobs',         $uri) ?>
            <?= el(url('/employer/candidates'),  'people',         'Candidates',      $uri) ?>
            <?= el(url('/employer/interviews'),  'calendar-check', 'Interviews',      $uri) ?>
        </ul>
        <div class="sidebar-section mt-3">Company</div>
        <ul class="list-unstyled mb-0">
            <?= el(url('/employer/profile'),      'building',      'Company Profile', $uri) ?>
            <?= el(url('/employer/notifications'),'bell',          'Notifications',   $uri) ?>
        </ul>
        <div class="sidebar-section mt-3">Account</div>
        <ul class="list-unstyled mb-0">
            <?= el(url('/account/settings'),     'gear',           'Settings',        $uri) ?>
        </ul>
        <div class="sidebar-section mt-3">Quick Action</div>
        <ul class="list-unstyled mb-0">
            <li>
                <a href="<?= url('/employer/jobs/create') ?>"
                   style="color:#fbbf24 !important;"
                   class="<?= str_contains($uri,'/jobs/create')?'active':'' ?>">
    ...
    <div class="p-3 border-top border-secondary mt-auto">
        ...
        <div class="d-flex align-items-center gap-2 mb-3">
            ...
            <div style="min-width:0;">
                ...
                <div class="small text-truncate" style="color:rgba(255,255,255,.45);font-size:11px;">Employer</div>
            </div>
        </div>
...
<div class="flex-grow-1 d-flex flex-column" style="min-width:0;">
    ...
    <nav class="navbar bg-white border-bottom d-lg-none px-3 py-2 sticky-top">
        ...
        <div class="d-flex gap-2">
            <a href="<?= url('/employer/notifications') ?>" class="btn btn-light btn-sm position-relative">
            ...
            <a href="<?= url('/employer/jobs/create') ?>" class="btn btn-primary btn-sm">
...
<nav class="bottom-nav">
    <a href="<?= url('/employer/dashboard') ?>"><i class="bi bi-speedometer2"></i>Home</a>
    <a href="<?= url('/employer/jobs') ?>"><i class="bi bi-briefcase"></i>Jobs</a>
    <a href="<?= url('/employer/jobs/create') ?>"><i class="bi bi-plus-circle-fill"></i>Post</a>
    <a href="<?= url('/employer/candidates') ?>"><i class="bi bi-people"></i>Candidates</a>
